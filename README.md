...
`PX_PER_CM = 156` 과 `cm()` 헬퍼를 씁니다. 1080x1920 세로 영상을 일반적인
스마트폰(가로 약 6.9cm)으로 볼 때의 환산값이라 `cm(0.3)` → 47px,
`cm(0.7)` → 109px 가 됩니다. 다른 크기 화면을 기준으로 잰 수치라면
`PX_PER_CM` 만 바꾸면 관련 배치가 함께 따라갑니다.

마무리 컷은 `UP = cm(0.7)` 하나로 전체를 위로 올립니다. 항목·합계·문장·
슬로건·로고가 모두 이 값을 빼서 배치되므로, 숫자만 바꾸면 컷 전체가
같이 움직입니다.
