...
```text
포돌이: podori.png 사용 (738x552 → 856x640)      ← 공식 파일 사용 중
포돌이: 대역 캐릭터 사용 (공식 파일을 ... 넣으면 교체됨)
```

세로 크기만 맞춰 리사이즈하므로 원본 가로세로 비율은 유지됩니다.
배경이 흰색인 이미지라면 투명 PNG로 만들어 넣어야 훅 화면에 네모난
흰 박스로 얹히지 않습니다.

**GitHub 웹에서 바로 올리는 방법** (가장 빠름):
`pol112ii/animation` → 브랜치 `claude/srt-whiteboard-animation-setup-eo2x50`
→ `assets/pm-enforcement/` 폴더 → **Add file → Upload files** →
`podori.png` 업로드 → Commit.

현재 커밋된 영상은 **대역 캐릭터** 버전입니다. 이 작업 환경의 이그레스
정책이 이미지 호스트를 차단해(`police.go.kr`, `wikimedia.org` 모두 차단)
공식 파일을 직접 내려받을 수 없었습니다. GitHub 도메인은 열려 있으므로
위 방법으로 올려주시면 그대로 반영됩니다.
